...
from app.services.station_manager import (
    assign_tracks_to_default_stations,
    rebuild_station_playlists,
)
    ...
    try:
        ...
        if request.file_path:
            background_tasks.add_task(
                _extract_single_file, request.file_path, request.update_existing, db
            )
...
async def scan_music_directory(
    background_tasks: BackgroundTasks, update_existing: bool = True
):
    """
    Scan the default music directory for audio files and extract metadata.
    This is a convenience endpoint for scanning the main music library.
    """
    try:
        configured_dir = settings.MUSIC_DIR or "/music"
        fallback_dir = "/mnt/music"

        candidate_dirs = [configured_dir]
        if fallback_dir not in candidate_dirs:
            candidate_dirs.append(fallback_dir)

        music_dir = next(
            (path for path in candidate_dirs if os.path.exists(path)), None
        )
...
async def get_file_metadata(file_id: int, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        return {
            ...
            "last_played_at": track.last_played_at.isoformat()
...
async def refresh_file_metadata(
    file_id: int, background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db)
):
    ...
    try:
        ...
        background_tasks.add_task(
            _refresh_track_metadata, track.id, track.file_path, db
        )
...
async def _extract_directory(
    directory_path: str, recursive: bool, update_existing: bool, db: AsyncSession
):
    ...
    try:
        ...
        metadata_list = await MetadataExtractor.scan_directory(
        ...
        logger.info(
            "Completed directory extraction",
            directory=directory_path,
        ...
        logger.error(
            "Failed directory extraction", directory=directory_path, error=str(e)
        )
        await db.rollback()
...
async def _update_or_create_track(
    metadata: Dict[str, Any], update_existing: bool, db: AsyncSession
) -> str:
    ...
    if track:
        ...
        track = Track(
            ...
            tags=metadata,
            is_christmas=bool(metadata.get("is_christmas")),
        )
        db.add(track)
        return "created"
...
def _update_track_from_metadata(track: Track, metadata: Dict[str, Any]):
    ...
    track.tags = metadata
    if "is_christmas" in metadata:
        track.is_christmas = bool(metadata["is_christmas"])
